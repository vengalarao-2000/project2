...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(
      collection(db, `users/${user.uid}/session_items`),
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setInProgressItems(
        allItems.filter(
          (i) => i.status === "analyzing" || i.status === "ready_for_selection"
        )
      );
    });
    return () => unsubscribe();
  ...
  async function openPicker() {
    try {
      setError("");
      setPicking(true);
      const res = await fetch(`${API_BASE}/api/picker/create-session`, {
        credentials: "include",
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to create picker session: ${res.status}`
        );
      }

      const data = await res.json();
      const { pickerUri, sessionId } = data;
      ...
      setPicking(false);
      setError(
        e.message || "Failed to open Google Photos picker. Please try again."
      );
    }
  }
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (pollCount > MAX_POLLS) {
          ...
          setPicking(false);
          setError("Photo selection timed out. Please try again.");
          if (popupWindow && !popupWindow.closed) popupWindow.close();
          return;
        }

        const res = await fetch(
          `${API_BASE}/api/picker/poll-session?sessionId=${sessionId}`,
          { credentials: "include" }
        );
        ...
        clearInterval(pollingRef.current);
        setPicking(false);
        setError("Failed to retrieve selected photos. Please try again.");
        if (popupWindow && !popupWindow.closed) popupWindow.close();
      }
    }, 2000);
  }
  ...
  const handleLocalUpload = async (event) => {
    ...
    // File size limit: 10MB per file (base64 increases size by ~33%, so ~7.5MB raw)
    const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB in bytes
    const invalidFiles = files.filter((file) => file.size > MAX_FILE_SIZE);

    if (invalidFiles.length > 0) {
      setError(
        `${invalidFiles.length} file(s) exceed the 10MB limit. Please upload smaller images.`
      );
      return;
    }
  ...
  const toggleAll = () => {
    setSelectedIds((prev) =>
      prev.size === unprocessedItems.length
        ? new Set()
        : new Set(unprocessedItems.map((i) => i.id))
    );
  };
      ...
      <main className="container py-4">
        ...
        <div className="d-flex justify-content-between align-items-center mb-5 pb-3 border-bottom">
          <div>
            ...
            <p className="text-muted mb-0">
              Manage your photo pipeline: Upload &rarr; Review &rarr; Publish
            </p>
          </div>
          ...
          <div className="d-flex gap-2">
            ...
            >
              ...
              {picking ? "Waiting..." : "Pick from Google Photos"}
            </button>
          </div>
        </div>

        {error && (
          <div className="alert alert-danger mb-4 shadow-sm">{error}</div>
        )}
        ...
        <section className="mb-5">
          ...
          {unprocessedItems.length > 0 ? (
            ...
            <EmptySection message="Your upload queue is empty. Add more photos using direct upload or google photos" />
          )}
        </section>
        ...
        <section className="mb-5">
          ...
          {inProgressItems.length > 0 ? (
            ...
            <EmptySection message="No items pending review. Select Unprocessed items above and click 'Generate Analysis'." />
          )}
        </section>
        ...
        <section>
          ...
          <div className="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
            <h4 className="h5 text-dark fw-bold mb-0">
              3. Generated Results{" "}
              <span className="badge bg-dark ms-2 rounded-pill">
                {processed.length}
              </span>
            </h4>
          ...
          {loadingHistory ? (
            ...
            <EmptySection message="No generated results yet. Complete the review step to see your final assets here." />
          )}
        </section>
      </main>
    </div>
